<template>
  <div class="bg-box">
    <div class="bg-img">
      <div class="bg-cover">
          <div class="api-box">
              <div class="api-title" :class="{rightshow:rightshow}"><span>真寻的api地址</span></div>
              <div class="api-input"><input v-model.trim="apiurl" @focus="inpOnfocus" @blur="inpOnBlur" type="text" placeholder="请输入你的真寻api地址" spellcheck="false"></div>
              <div class="tips">
                <p>注意事项：</p>
                <p>①开发环境中修改api地址一样生效，即覆盖代理服务器的转发</p>
                <p>②如果生产环境部署的资源和真寻本体<ins><b>在</b></ins>同一个机器上，则输入空。如果修改过真寻的默认端口，则输入<ins><b>“http://localhost:你的真寻端口”</b></ins></p>
                <p>③如果生产环境部署的资源和真寻本体<ins><b>不在</b></ins>同一个机器上，则输入你的服务器接口地址以及真寻的端口。最后记得修改防火墙设置哦</p>
              </div>
              <div class="api-btn">
                  <button class="goback"  @click="goBack">返回</button>
                  <button class="myapi" ref="myapi" @click="changeUrl($event)">修改</button>
              </div>
          </div>
      </div>
    </div>
...
<style scoped>
  *{
  ...
  .myapi:hover, .goback:hover{
      background-color: #289045;
  }
  .myapi:hover.myapi::before, .goback:hover.goback::before{
      top:0;
  }
  .tips{
    margin: 0 2rem;
